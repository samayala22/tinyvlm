...
void UVLM_2DOF::run(const UVLM_2DOF_Vars& vars, f32 t_final_nd) {
    ...
    for (const i32 i : tiny::pbar(0, (i32)t_steps-2)) {
        ...
        while (backend->blas->norm(du_k.view()) > EPS_f) {
            ...
            if (i == 0 || i == 1) {
                for (i64 i = 0; i < m_assembly.num_surfaces(); i++) {
                    std::cout << aero_forces.views()[i] << "\n";
                }

                std::cout << F_h.view().slice(All, i+1) << "\n";
            }
