...
C     FILE CONTAINING PARAMETERS
      ...
      PARAMETER (NWMAX=100)
C     ALF - SECTIONAL ANGLE OF ATTACK
      DIMENSION ALF(NCMAX+1)
      DIMENSION SNO(NCMAX+1)
...
C     NSTEPS = NO. OF TIME STEPS.
      PAY=3.141592654
      RO=1.
C     BH = HEAVING AMPLITUDE, OM = FREQUENCY, VINF = FAR FIELD VELOCITY
      BH=0.1
      OM=1.0
      VINF=1.0
C     C=CHORD; C=ROOT CHORD, B=SEMI SPAN, DX,DY=PANEL DIMENSIONS, CH=GROUND CLEARANCE
      C=1.
      B=2.0
      DX=C/NC
      DY=B/NS
...
C Angle of attack and slip angle
      ALFA1=-5.0
      ALFAO=0.0
...
C     GEO CALCULATES WING COLLOCATION POINTS CP,AND VORTEX TIPS QF
 ...
 4    WRITE(6,105) I,BB(I)
      ...
      OPEN(UNIT=10, FILE='katz_cl.txt', STATUS='UNKNOWN')
