...
VLM::VLM(const std::string& backend_name, const std::string& mesh_name) : Simulation(backend_name, mesh_name), 
    lhs(backend->allocator), rhs(backend->allocator), gamma(backend->allocator), gamma_prev(backend->allocator), delta_gamma(backend->allocator), local_velocities(backend->allocator) {
    mesh.alloc_wake(1); // single wake panel
    mesh.verts_wing_init.to_device();
    alloc_buffers();
};

void VLM::alloc_buffers() {
    lhs.alloc(mesh.nb_panels_wing() * mesh.nb_panels_wing());
    rhs.alloc(mesh.nb_panels_wing());
    gamma.alloc(mesh.nb_panels_total());
    gamma_prev.alloc(mesh.nb_panels_wing());
    delta_gamma.alloc(mesh.nb_panels_wing());
    local_velocities.alloc(mesh.nb_panels_wing() * 3);
}

AeroCoefficients VLM::run(const FlowData& flow) {
    // Reset buffer state
    backend->allocator.d_memset(lhs.d_ptr(), 0, lhs.size());
    backend->allocator.dd_memcpy(mesh.verts_wing.d_ptr(), mesh.verts_wing_init.d_ptr(), mesh.verts_wing.size());
    mesh.nwa = 0;
    ...
    backend->mesh_move(mesh, init_pos);
}
