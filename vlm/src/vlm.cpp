...
Simulation::Simulation(const std::string& backend_name, const std::vector<std::string>& meshes, bool legacy) : backend(create_backend(backend_name)) {
    ...
    for (i64 i = 0; i < meshes.size(); i++) {
        ...
        mesh_io.read(meshes[i], verts_wing_init_h.views()[i], legacy);
    }
