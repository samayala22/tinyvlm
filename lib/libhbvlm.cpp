...
)
{
    ...
    Tensor2dD m_G_buf_k{backend->memory.get()}; // copy because lsq overwrites
    Tensor1dD m_x_curr{backend->memory.get()};
    Tensor1dD m_x_new{backend->memory.get()};
    Tensor1dD m_g_curr{backend->memory.get()};
    Tensor1dD m_g_new{backend->memory.get()};
    Tensor1dD m_gamma{backend->memory.get()};
